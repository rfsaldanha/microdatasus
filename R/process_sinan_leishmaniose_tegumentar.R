...
#'
#' Recodes supported fields from SINAN tegumentary leishmaniasis notifications
...
#'   `information_system = "SINAN-LEISHMANIOSE-TEGUMENTAR"`, or a compatible
#'   layout.
...
process_sinan_leishmaniose_tegumentar <- function(
  ...
  if ("CLI_CUTANE" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        CLI_CUTANE = dplyr::recode_values(
          .data$CLI_CUTANE,
          "1" ~ "Sim",
          "2" ~ "N\u00e3o",
          "9" ~ NA,
  ...
  if ("CLI_MUCOSA" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        CLI_MUCOSA = dplyr::recode_values(
          .data$CLI_MUCOSA,
          "1" ~ "Sim",
          "2" ~ "N\u00e3o",
          "9" ~ NA,
  ...
  if ("CLI_CICATR" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        CLI_CICATR = dplyr::recode_values(
          .data$CLI_CICATR,
          "1" ~ "Sim",
          "2" ~ "N\u00e3o",
          "9" ~ NA,
  ...
  if ("CLI_CO_HIV" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        CLI_CO_HIV = dplyr::recode_values(
          .data$CLI_CO_HIV,
          "1" ~ "Sim",
          "2" ~ "N\u00e3o",
          "9" ~ NA,
  ...
  if ("LAB_PARASI" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        LAB_PARASI = dplyr::recode_values(
          .data$LAB_PARASI,
          "1" ~ "Positivo",
          "2" ~ "Negativo",
          "9" ~ NA,
  ...
  if ("LAB_HISTOP" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        LAB_HISTOP = dplyr::recode_values(
          .data$LAB_HISTOP,
  ...
  if ("CLA_TIPO_N" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        CLA_TIPO_N = dplyr::recode_values(
          .data$CLA_TIPO_N,
          "1" ~ "Caso novo",
          "2" ~ "Recidiva",
          "3" ~ "Transfer\\u00eancia",
  ...
  if ("CLAS_FORMA" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        CLAS_FORMA = dplyr::recode_values(
          .data$CLAS_FORMA,
          "1" ~ "Cut\\u00e2nea",
          "2" ~ "Mucosa",
          "9" ~ NA,
  ...
  if ("TRA_DROGA_" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        TRA_DROGA_ = dplyr::recode_values(
          .data$TRA_DROGA_,
  ...
  if ("TRA_OUTR_N" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        TRA_OUTR_N = dplyr::recode_values(
          .data$TRA_OUTR_N,
          "1" ~ "Antimonial Pentavalente",
          "2" ~ "Anfotericina b",
          "3" ~ "Pentamidina",
  ...
  if ("CON_CLASS_" %in% variables_names) {
    data <- data %>%
      dplyr::mutate(
        CON_CLASS_ = dplyr::recode_values(
          .data$CON_CLASS_,
          "1" ~ "Aut\\u00f3ctone",
          "2" ~ "Importado",
          "3" ~ "Indeterminado",
