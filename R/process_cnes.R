...
process_cnes <- function(data,
  ...
  if("ATIVIDAD" %in% variables_names){
    data[, ATIVIDAD := data.table::fcase(
  ...
  if("CLIENTEL" %in% variables_names){
    data[, CLIENTEL := data.table::fcase(
  ...
  if("TURNO_AT" %in% variables_names){
    data[, TURNO_AT := data.table::fcase(
  ...
  if("NATUREZA" %in% variables_names){
    data[, NATUREZA := data.table::fcase(
  ...
  if("TPGESTAO" %in% variables_names){
    ...
    data[, TPGESTAO := data.table::fcase(
      TPGESTAO == "D", "Dupla",
      TPGESTAO == "E", "Estadual",
  ...
  if("ESFERA_A" %in% variables_names){
    data[, ESFERA_A := data.table::fcase(
      ESFERA_A == 1, "Federal",
      ESFERA_A == 2, "Estadual",
  ...
  if("NIV_HIER" %in% variables_names){
    data[, NIV_HIER := data.table::fcase(
      NIV_HIER==1, "PAB-PABA",
      NIV_HIER==2, "M\u00e9dia M1",
  ...
  if("RETENCAO" %in% variables_names){
    data[, RETENCAO := data.table::fcase(
...
#--------------------------------------------------------------------------------------------------------------
  if(information_system == "CNES-ST"){
    ...
    if ("CODUFMUN" %in% variables_names & municipality_data == TRUE) {
      data[, CODUFMUN := as.numeric(as.character(CODUFMUN))]
    ...
    convert_dummy_to_sim_nao(data = data, column_names = cols_to_sim_nao)


    # COD_IR
    if("COD_IR" %in% variables_names){
      data[, COD_IR := data.table::fcase(
    ...
    if("TP_PREST" %in% variables_names){
      data[, TP_PREST := data.table::fcase(
    ...
    if("ORGEXPED" %in% variables_names){
      data[, ORGEXPED := data.table::fcase(
        ORGEXPED==1, "SES",
        ORGEXPED==2, "SMS"
        )]
    ...
    if("CLASAVAL" %in% variables_names){
      data[, CLASAVAL := data.table::fcase(
...
# ------------------------------------------------------------------------------
    ...
    if ("UFMUNRES" %in% variables_names & municipality_data == TRUE) {
      ...
      convert_to_character(data=data, column_names = "UFMUNRES")
    }


    # TERCEIRO
    if("TERCEIRO" %in% variables_names){
      data[, TERCEIRO := data.table::fcase(
        TERCEIRO==1, "Sim",
        TERCEIRO==0, "N\u00e3o",
        TERCEIRO==2, "N\u00e3o"
        )]
    ...
    #   data[, CPF_PROF := data.table::fcase(
    ...
    if("VINCULAC" %in% variables_names){
      data[, VINCULAC := as.character(as.numeric(as.character(VINCULAC)))]
      # data[, VINCULAC := data.table::fcase(
